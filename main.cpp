...
public:
  ...
  void putdata();
  void modify();
  bool booked_status() { return booked; }
  int get_ID() { return ID; }
  char *get_name() { return name; }
  void bookit() { this->booked = 1; }
  void unbookit() { this->booked = 0; }
} c;
class Node {
public:
  Cars car;
...
public:
    ...
    void add_car(Cars c) {
        ...
        if(root == NULL) {
            ...
            while(true) {
                if(c.get_ID() < current->car.get_ID()) {
    ...
    void display_cars() {
        ...
        display_cars_helper(root);
    }
private:
    void display_cars_helper(TreeNode* node) {
        if(node != NULL) {
            display_cars_helper(node->left);
            node->car.putdata();
            cout << endl;
            display_cars_helper(node->right);
        }
    }
};
class Login {
...
void Login_admin::Read_user_data() {
  ...
  while (fin >> username >> password) {
    cout << "|" << setw(20) << username << "|" << setw(10) << password << "|"
         << endl;
  }
  fin.close();
}
...
void Login_admin::modify_car() // fuction to modify the records of file
{
  ...
  while (!fio1.eof()) {
    ...
    if (c.get_ID() == entered_id) {
      ...
      cout << "\nYour record of ID " << entered_id << " has been updated"
           << endl;
      break;
    }
  }
...
void Login_user::book_hatchback() {
  ...
  cin >> id;
  int found = search_car_hatchback(id);
  if (found == -1) {
...
void Login_user::unbook() {
  ...
  if (found == 'y') {
    cout << "Hope You liked Our Hatchback Car - " << name_of_car << " :)"
         << endl;
...
void Login_user::bill(int carid) //(float rate,int carid,char* name_of_car)
{
  ...
  case 1:
    ...
    cout << "************************ BILL ********************************"
         << endl;
    cout << "|" << setw(30) << "Invoice Number"
         << "|" << setw(30) << bill_id << endl;
    cout << "|" << setw(30) << "Customer Name"
         << "|" << setw(30) << username << endl;
    cout << "|" << setw(30) << "Car Name"
         << "|" << setw(30) << namethecar << endl;
    cout << "|" << setw(30) << "Car ID"
         << "|" << setw(30) << carid << endl;
    cout << "|" << setw(30) << "Numbers of Hours"
         << "|" << setw(30) << n << endl;
    cout << "|" << setw(30) << "Total Pay"
         << "|" << setw(30) << pay << endl;
    cout << "|" << setw(30) << "Advance Pay "
         << "|" << setw(30) << min_pay << endl;
    cout << "|" << setw(30) << "After use Pay"
         << "|" << setw(30) << pay - min_pay << endl;
    cout << "|" << setw(30) << "Penalty per hour"
         << "|" << setw(30) << penalty_rate << endl;
    cout << "***************************************************************"
         << endl;
    break;
  case 2:
    // while(1)
    //{
    cout << "Enter number of days for which u want to get car on rent(max a "
            "month): ";
    cin >> n;
    // if(n>0&&n<32){break;}
...
void Login_user ::do_user_work() {
  ...
  case 1:
    ...
    cout << "|" << setw(10) << "TYPE"
         << " |" << setw(15) << "RATE(per hr)"
         << " |" << setw(20) << "PENALTY RATE(per hr)"
         << " |" << endl;
    cout << "|" << setw(10) << "HATCHBACK"
         << " |" << setw(15) << 50 << " |" << setw(20) << 50 * (0.1) + 50
         << " |" << endl;
    cout << "|" << setw(10) << "SEDAN"
         << " |" << setw(15) << 75 << " |" << setw(20) << 75 * (0.1) + 75
         << " |" << endl;
    cout << "|" << setw(10) << "SUV"
         << " |" << setw(15) << 85 << " |" << setw(20) << 85 * (0.1) + 85
         << " |" << endl;
    cout << "|" << setw(10) << "LUXURY"
         << " |" << setw(15) << 220 << " |" << setw(20) << 220 * (0.1) + 220
         << " |" << endl;
    cout << "\nBooking Point\n 1.Hatchback\n 2.Sedan\n 3.Suv\n 4.Luxury\n Your "
            "Choice: ";
    cin >> ch;
...
void Login_user ::login(char user_n[], char pass[]) {
  ...
  ifstream fin("login_data.txt", ios::in);
  while (fin >> username >> password) {
    if (strcmp(user_n, this->username) == 0 &&
        strcmp(pass, this->password) == 0) {
      name = conv_to_str(user_n, str_len(user_n));
